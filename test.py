...
def test(map, episodes, iterations, final_experiment, eps=1e-2):
    # map.configure(prey_reward, stuck_penalty, agent_max_age)
    # agents = map.get_agents()
    #directions = [(-1, 0), (0, -1), (1, 0), (0, 1), (0, 0)]
    times = 0
    map.A_mind.network.load_state_dict(torch.load("%s/network.pth" % (final_experiment), map_location=torch.device('cpu')))
    map.A_mind.target_network.load_state_dict(torch.load("%s/target_network.pth" % (final_experiment), map_location=torch.device('cpu')))
    map.A_mind.optimizer.load_state_dict(torch.load("%s/optimizer.pth" % (final_experiment), map_location=torch.device('cpu')))
    #map.A_mind.eval()
...
if __name__ == '__main__':
    [_, name, iterations, init_conc, target_conc, agent_range, final_experiment] = sys.argv
    ...
    test(society((50, 50), init_conc=float(init_conc), target_conc=float(target_conc), agent_range = int(agent_range), 
                 name=name, max_iteration = int(iterations), lock=l), episodes, iterations, final_experiment)
